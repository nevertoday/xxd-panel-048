---
name: xxd-panel-048
description: "Create XXD Panel 048 artwork from supplied photos in four combinable modes: photo paired with a source-specific transparent structural blueprint, blueprint alone, or a four-device wallpaper pack with independent or anchor-linked continuity. Use for the exact 048 scientific-editorial style; reject generic line art, decorative HUDs, arbitrary machinery, anatomical gore, fixed-blue templates, dense dashboards, dark cyberpunk, and unreadable pseudo-technical text."
---

# XXD Panel 048 · 透明结构蓝图

Rebuild the recognisable subject as a lucid structural blueprint between engineering drawing, X-ray transparency, scientific illustration, and a future archive. Explain internal logic according to what the subject actually is, while preserving identity and editorial restraint.
...
**lock identity, silhouette, pose, and narrative relation → preserve three cues → decide what “inside” means for this subject → reveal only the most explanatory layers → organise transparency, cutaway, exploded detail, axes, leaders, and scale into one reading path → derive a luminous monochrome system from the source → keep generous clean space → bind technical copy to exact structures**.

Reject the result as generic if an unrelated photograph could replace the source without materially changing recognition, explanatory structure, layer logic, annotations, colour, whitespace, or copy. The operative exclusions are: ordinary outline tracing, arbitrary component piles, anatomical gore, generic blueprint templates, fixed cobalt blue, dense sci-fi HUD, neon cyberpunk, multicolour dashboards, and annotation noise.

## 048 visual system

- Preserve at least three cues across silhouette, proportion, pose, axis, direction, opening, material, action, function, distance, or relation.
- Keep the outer contour immediately recognisable, then reveal a selective internal logic: components and joints for objects, space and construction for architecture, propulsion and chassis for vehicles, veins and growth for plants, and pose skeleton, motion path, or clothing layers for people and animals—never flesh-like dissection.
- Use partial transparency, section cuts, layered ghosting, exploded fragments, magnified details, axes, dimensions, numbered nodes, leaders, and small diagrams only when they explain the subject. Differentiate hierarchy by line weight, opacity, density, and focus.
- Build one clear visual centre and one reading path with generous blank space. Information must orbit the subject rather than fill the frame as a dashboard.
- Derive one clean, luminous, businesslike monochrome family from the source's most characteristic colour. A light ground, one principal line colour, and limited tonal steps are enough; never default to blue.
- Preserve the tension between precision and editorial calm: the image must feel structurally rational without becoming literal CAD, a medical plate, or futuristic interface decoration.

## Copy belongs to the image language

Use one concise title derived from identity, function, state, action, or symbolic meaning, plus only essential component names, status words, direction notes, dimensions, numbers, or micro-annotations. Attach native type to a real leader, scale, node, section, axis, or edge so every label has an explanatory role. Do not introduce a year or long explanatory copy.
...
Use only current attachments, explicit paths, or a previously supplied image clearly identified by the user. Never scan Desktop, `~/Desktop/xxd/xxd-panel-048/`, or historical task folders for a substitute.
...
1. Resolve one or more modes. In a native multi-select use these human-facing labels: `上下对照`, `左右对照`, `只要设计图`, and `四端壁纸`. If no real multi-select exists, ask and wait:
   ...
   1. 上下对照（原图在上，048 设计在下）
   2. 左右对照（原图在左，048 设计在右）
   3. 只要设计图（成品中不显示原图）
   4. 四端壁纸
      手机、iPad、电脑、儿童手表各一张
...
5. Start a fresh job and reserve collision-safe output directories before generation. Use only the current invocation's explicit source or theme. Read `references/xxd-panel-048-prompt.en.md` or `references/xxd-panel-048-prompt.zh-CN.md` immediately before building the generation request.
...
7. Use **single complete-canvas generation as the default for every mode**:
   - `top-bottom`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source in the upper half and the 048 transformation in the lower half.
   - `left-right`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source on the left and the 048 transformation on the right.
   - `design-only`: generate the 048 transformation across the whole canvas; the source is reference-only and not visible.
...
10. Inspect the actual bitmap at full size and thumbnail size. Check, in order: whole-poster integration; 048 aesthetic fidelity; source identity and structure; visual and semantic correspondence between regions; typography and locale; mode, ratio/pixels, count, and PNG format; then approximate 50/50 geometry.
...
COPY PAYLOAD: <the exact 048-specific title and supporting-text package resolved under the local production prompt>
COPY RULE: Render only the populated strings in COPY PAYLOAD, each exactly once. Do not rewrite, translate, spell-correct, duplicate, or add text. Use native shaping, direction, punctuation, spacing, and semantic line breaks. Preserve the 048-specific hierarchy, amount of supporting text, placement, material, and typographic role instead of applying a generic overlay.
```
...
`scripts/compose_panel.py` remains available for deterministic recovery and audit. Trigger it only under step 11. When used, generate a same-aspect design asset from the full 048 prompt, preserve the source without destructive crop or stretch, and document which fallback condition applied. A direct complete-canvas success must not be split and recomposed again.
...
```text
~/Desktop/xxd/xxd-panel-048/<fresh-task>/
└── source-01/
...
Preserve explicit subject, mode set, output count, exact pixels or ratios, wallpaper relationship, copy mode, exact wording, locale, and intended meaning. User overrides may alter these variables but do not silently authorise abandoning source identity, the 048 aesthetic motive, fresh-task isolation, raster-only output, privacy, or verification. If the user explicitly requests a different aesthetic, acknowledge that it leaves this Skill's style rather than pretending it remains 048.

## References

- Read `references/xxd-panel-048-prompt.en.md` or `references/xxd-panel-048-prompt.zh-CN.md` immediately before generation.
- `references/048-source.md` archives the original style brief and is evidence, not an implicit 3:4 default.
